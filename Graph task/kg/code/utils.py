...
def set_logger(args):
    ...
    # 安全的时间戳格式：2025-07-09_17-32-04
    now_time = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    if args.do_train:
        log_file = os.path.join(args.save_path or args.init_checkpoint, f"{now_time}_train_{args.model}.log")
    else:
        log_file = os.path.join(args.save_path or args.init_checkpoint, f"{now_time}_test_{args.model}.log")
...
def read_from_id(filename='../data/FB15k-237/entity2id.txt'):
    entity2id = {}
    id2entity = {}
    with open(filename) as f:
        for line in f:
...
def build_data(path):
    ...
    train_triples = load_triples_from_txt(filename=os.path.join(folder, 'train.txt'), parse_line=parse_line,
                                          entity2id=entity2id, relation2id=relation2id)

    valid_triples = load_triples_from_txt(filename=os.path.join(folder, 'valid.txt'), parse_line=parse_line,
                                          entity2id=entity2id, relation2id=relation2id)

    test_triples = load_triples_from_txt(filename=os.path.join(folder, 'test.txt'), parse_line=parse_line,
                                         entity2id=entity2id, relation2id=relation2id)
